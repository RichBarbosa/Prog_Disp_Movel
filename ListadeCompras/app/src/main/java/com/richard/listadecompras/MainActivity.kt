...
class MainActivity : AppCompatActivity(), CompraAdapterListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        edtCompra = findViewById(R.id.edtTarefa)

        btnIncluir.setOnClickListener() {
            if (edtCompra.text.toString().isNotEmpty()) {
                adicionarTarefa(edtCompra.text.toString())
    ...
    fun adicionarTarefa(nomeTarefa: String) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            db?.compraDao()?.addCompra(Compra(nome = nomeTarefa))
            ...
            val resposta = compraDAO?.getCompra()
    ...
    override fun excluirCompra(compra: Compra) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            // A partir do DAO executa uma ação escolhida (SELECT)
            val resposta = tarefaDAO?.getCompra()
